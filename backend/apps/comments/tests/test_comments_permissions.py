...
class CommentPermissionsTests(TestCase):
    def setUp(self):
        ...
        # Налаштовуємо структуру obj.owner.user (як у вашому permissions.py)
        # Якщо в CommentModel поле називається author, пермішен варто переписати на obj.author
        # Але тут ми тестуємо саме код пермішена:
        self.obj.owner.user = self.user

    def test_is_owner_or_admin(self):
        """Власник або суперюзер має доступ"""
        # 1. Власник
    ...
    def test_is_admin_or_superuser(self):
        """Тільки суперюзер має доступ (has_permission)"""
        # 1. Суперюзер
        self.request.user = self.admin
